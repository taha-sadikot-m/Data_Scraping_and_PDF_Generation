...
class WebScraper:
    """Class to handle web scraping and table extraction"""
    ...
    def scrape_url(self, url: str) -> Tuple[BeautifulSoup, List[TableData]]:
        """
        Scrape the given URL and extract all tables

        Args:
            url: The URL to scrape

        Returns:
            Tuple of (BeautifulSoup object, List of TableData objects)
        """
        try:
            ...
            tables = self._extract_tables_with_pandas(url)

            # If pandas fails, fallback to BeautifulSoup
            if not tables:
                tables = self._extract_tables_with_bs4(soup, url)
    ...
    def _extract_tables_with_pandas(self, url: str) -> List[TableData]:
    ...
    def _extract_tables_with_bs4(self, soup: BeautifulSoup, url: str) -> List[TableData]:
...
class LLMManager:
    """Manages multiple LLM API interactions"""
    ...
    async def generate_content_for_table(self, table_data: TableData) -> Dict[str, List[str]]:
        ...
        Args:
            table_data: TableData object containing the table information

        Returns:
            Dictionary mapping LLM names to lists of generated paragraphs
        """
        results = {}
...
class PDFGenerator:
    """Generates PDF documents from the generated content"""
    ...
    def generate_pdf(self, url: str, tables_data: List[TableData], 
        ...
        Args:
            url: Source URL
            tables_data: List of TableData objects
            llm_results: Nested dict {llm_name: {table_index: [paragraphs]}}
        ...
        for table_data in tables_data:
            ...
            if table_idx in llm_content:
                for i, paragraph in enumerate(llm_content[table_idx]):
                    #story.append(Paragraph(f"<b>Row {i + 1}:</b>", self.styles['Normal']))
...
def setup_llm_configs() -> List[LLMConfig]:
    ...
    if hf_api_key:
        # Add multiple HuggingFace models
        hf_models = [
...
async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Web Table Scraper and LLM Document Generator")
